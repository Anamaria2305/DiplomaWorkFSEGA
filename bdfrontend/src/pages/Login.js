...
export default function Login() {
    ...
    async function handleSubmit(event) {
      ...
      try {
        var response= await axios.post("http://localhost:8080/getAuthority",{username: uname.value, password: pass.value})
        var response2= await axios.post("http://localhost:8080/login",{username: uname.value, password: pass.value})
       ...
       if( JSON.parse(localStorage.getItem("role"))[0].authority === "admin" )
        navigate("/adminpage")
        if( JSON.parse(localStorage.getItem("role"))[0].authority === "doctor" )
        navigate("/doctorpage")
    ...
    return (
        <div className="App2"  >    <Navbar></Navbar>
            ...
            <Form  onSubmit= {handleSubmit}>
                <Form.Group>
                    <Form.Label>Introduceti adresa de e-mail(donator) sau username-ul (doctor) :</Form.Label>
                    <Form.Control type="text" required name= "uname"
                        placeholder="Adresa de e-mail/username"/>
                </Form.Group>
                <Form.Group>
